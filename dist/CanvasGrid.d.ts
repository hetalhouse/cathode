...
export interface FilterPopupLayout {
    box: OverlayBox;
    input: OverlayBox;
    clear: OverlayBox | null;
}
/** Canvas-space popup geometry, anchored under the column's header, clamped to the panel. */
export declare function layoutFilterPopup(canvasW: number, anchorX: number, hasValue: boolean): FilterPopupLayout;
/** Which popup zone a canvas point is over. `scale` widens the ✕ like the header icons. */
export declare function hitFilterPopup(cx: number, cy: number, l: FilterPopupLayout, scale?: number): 'input' | 'clear' | 'inside' | 'outside';
/** Draw the popup: panel + value/placeholder + caret + optional ✕. Value overflow keeps the TAIL visible (that's where the caret is). */
export declare function drawFilterPopup(ctx: CanvasRenderingContext2D, l: FilterPopupLayout, value: string, caretOn: boolean, colors: GridColors): void;
/**
...
export declare function isOnFilterIcon(cx: number, colStartX: number, colWidth: number, scale?: number): boolean;
...
export declare function isOnResizeHandle(cx: number, colStartX: number, colWidth: number, scale?: number): boolean;
